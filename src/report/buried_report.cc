...
// 创建一个定时器，约定5s后调用ReportCacher_方法
...
// 首先切线程，统一放到ReportStrand模块调度任务，防止不同Strand产生数据竞争的问题
...
// 通过db对象从数据库按优先级取10条数据，再通过GenReportData_构造网络上报的body
// 再调用ReportData_上报数据，上报成功后通过db对象从数据库删除对应数据，清空data_caches_，防止重复上报
// 然后调用NextCycle_启动下一次定时任务
...
// 先解密，然后通过json构造json array，最后序列化成string作为上报的body
...
// 用户传入数据 + 公共数据构造json object，序列化后加密，再通过db对象存入数据库
